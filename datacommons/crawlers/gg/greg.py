...
def search(context, data):
    ...
    form[
        "ctl00$ScriptManager2"
    ] = "ctl00$cntPortal$updPanel|ctl00$cntPortal$btnSearch"  # noqa
...
def clean_data(context, data):
    name = (
        data.get("company_name") or data.get("llp_name") or data.get("foundation_name")
    )
    ...
    data["name"] = name
    date = (
        data.get("company_registered_date")
