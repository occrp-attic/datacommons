...
def parse_declaration(context, decl):
    ...
    full_name = ' '.join([infocard.get('last_name', ''),
                          infocard.get('first_name', ''),
                          infocard.get('patronymic', '')])
    person_id = infocard.get('id', make_key(full_name,
                                            decl.get('position', '')))
        ...
        for idn, smth in data.items():
            ...
            if isinstance(rights, dict):
                for r_id, right in rights.items():
                    ...
                    audit(context, right)
                    context.emit(rule=STEPS.get(num) + '_rights',
                                 data=right)
            if isinstance(guarantors, dict):
                for g_id, guarantor in guarantors.items():
                    ...
                    context.emit(rule=STEPS.get(num) + '_guarantors',
                                 data=guarantor)
            if isinstance(guarantor_realty, dict):
                for g_id, grealty in guarantor_realty.items():
                    ...
                    context.emit(rule=STEPS.get(num) + '_guarantor_realty',
                                 data=grealty)
            audit(context, smth)
            context.emit(rule=STEPS.get(num),
                         data=smth)
    audit(context, infocard)
    context.emit(rule='person',
                 data=infocard)
